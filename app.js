...
ons.ready(function() {
	...
	if(isDebug()){		
		...
		setStorage("search_address","Valencia, Carabobo, Venezuela");		
	}
...
document.addEventListener("pageinit", function(e) {
	...
	{		
	   ...
	   case "page-paymentoption":
	     translatePage();
	     $(".order_change").attr("placeholder", getTrans('Numero de referencia','order_change') );


	     $("#report_payment").attr("value", getTrans('Report Payment','report_payment') );
		 break;
	  ...
	  case "page-addressbook-details":
	   ...
	   case "page-signup":
	   	 $('[type="date"].birthday').prop('max', function(){
		  	return new Date().toJSON().split('T')[0];
		  });
	   	 /*NUEVO*/
		  $('.identification_card').mask('00000000', {'translation': {
			    /*R: {pattern: /(E|V)/},  condiciona a que antes de introducir la cedula tenga una letra (esto va en el mask R00000000) */
			    '0': {pattern: /[0-9]/}
			  }
			});
	     translatePage();
	     translateValidationForm();
	     $(".first_name").attr("placeholder",  getTrans("Nombre",'first_name') );
	     $(".last_name").attr("placeholder",  getTrans('Apellido','last_name') );
	     $(".birthday").attr("placeholder",  getTrans('Fecha de Nacimiento','birthdate') );
	     $(".identification_card").attr("placeholder",  getTrans('Cédula de Indentidad','identification_card') );
	     /*VALIDACION DE TELEFONO*/
	     $('.contact_phone').mask('00000000000', {'translation': {
			    '0': {pattern: /[0-9]/}
			  }
			});
	     /*$(".contact_phone").attr("placeholder",  getTrans('Teléfono Móvil','contact_phone') );*/
	     $(".email_address").attr("placeholder",  getTrans('Correo Electrónico','email_address') );
	     $(".password").attr("placeholder",  getTrans('Contraseña','password') );
	     $(".cpassword").attr("placeholder",  getTrans('Confirmar Contraseña','confirm_password') );	     
	   ...
	   case "page-shipping":
	   	  ...
	   	  var customer_id_card = getStorage("customer_id_card");
	   	  if (typeof customer_id_card === 'undefined' || customer_id_card==null || customer_id_card=="" || customer_id_card == 'null'){
		   	  /*NUEVO*/
			  $('.identification_card').mask('R00000000', {'translation': {
				    R: {pattern: /(E|V)/},
				    '0': {pattern: /[0-9]/}
				  }
				});
			  $(".identification_card").attr("placeholder", getTrans('Identification Card','identification_card2') );
	   	  }else{
		...
		/*Inicio Nueva funcion menu*/
		$('#menu-lower').show();
		
		/*Fin Nueva funcion menu*/
		...
		case "page-home":
		    ...
		    if ( search_mode=="postcode"){		
		    	...
		    	if(enabled_advance_search=="yes"){
		    		...
		    		var tab_html='';
		    		 tab_html+='<ons-tabbar position="top" class="menu_tabbar" >';
		    		 	if(theme_address!=2){
						  tab_html+='<ons-tab page="page_search.html" icon="ion-android-pin" active="true"></ons-tab>';
						}
						if(theme_name!=2){ 
						  tab_html+='<ons-tab page="page_search_byname.html" icon="ion-android-restaurant"></ons-tab>';
						}
						if(theme_street_name!=2){
						  tab_html+='<ons-tab page="page_search_bystreet.html" icon="ion-android-map"></ons-tab>';
						}
						if(theme_cuisine!=2){
						  tab_html+='<ons-tab page="page_search_bycuisine.html" icon="ion-pizza"></ons-tab>';
						}
						if(theme_foodname!=2){
						  tab_html+='<ons-tab page="page_search_byfood.html" icon="ion-coffee"></ons-tab>';
						}
					 tab_html+='</ons-tabbar>';
			...
			if (day_night>=6 && day_night<12){
				$('.page--back__background').css('background','transparent url(css/images/bg-d.png) no-repeat scroll center center / cover');
			}else if(day_night>=12 && day_night<=18){
				$('.page--back__background').css('background','transparent url(css/images/bg-t.png) no-repeat scroll center center / cover');
			}else{
				$('.page--back__background').css('background','transparent url(css/images/bg-n.png) no-repeat scroll center center / cover');
			}
		...
		case "page-colifavor":
		  ...
		  "client_token="+getStorage("client_token")
			...
			if (day_night>=6 && day_night<=18){
				$('.img-serv-wrap').prepend('<img src="css/images/day.png" alt="" title="" style="width: 100%;">');
			}
			else{
				$('.img-serv-wrap').prepend('<img src="css/images/night.png" alt="" title="" style="width: 100%;">');
			}
		...
		case "page-colitaxi":
		  ...
		  "client_token="+getStorage("client_token")
			...
			var day_night=current.getHours();
			if (day_night>=6 && day_night<=18){
				$('.img-serv-wrap').prepend('<img src="css/images/day_taxi.png" alt="" title="" style="width: 100%;">');
			}
			else{
				$('.img-serv-wrap').prepend('<img src="css/images/night_taxi.png" alt="" title="" style="width: 100%;">');
			}
		...
		case "page-profile":
		  ...
		  /*NUEVO*/
		  $(".identification_card").attr("placeholder",  getTrans("Identification Card",'identification_card2') );
		  $(".birthday").attr("placeholder",  getTrans("Birthdate",'birthdate') );
		...
		case "page-shipping-location-area":   
		 ...
		 case "page-delivery-opt-page":
		  ...
		  var cart_params=JSON.stringify(cart);       	        	  
      	  ...
      	  callAjax("loadDeliveryOpt",params);
...
function applyFoodFilters(tipo){
    if (tipo){
    var params2;
    var search_mode2;
        
    params2 = "address="+ getStorage("search_address") +
	"&cuisine_type="+tipo;
	
	search_mode2 = getSearchMode();
	if ( search_mode2=="postcode"){
		params2+="&search_mode="+ search_mode2;
		search_type = getSearchType();
		switch (search_type)
		{
			case "1":				
			params2+="&city_id="+ global_city_id;
			params2+="&area_id="+ global_area_id;
			break;
			
			case "2":
			params2+="&state_id="+ global_state_id;
			params2+="&city_id="+ global_city_id;
			break;
			
			case "3":
			params2+="&postal_code="+ global_postal_code;
			break;
        ...
        global_filter_params = params2;

        if( tipo == 36 ){

        	var birthdate=getStorage("customer_birthday");
			if(!empty(birthdate)){
				console.log(birthdate);
				var strbirth = birthdate.split("-");
				var dob = new Date(strbirth);
				if(calculate_age(dob)>=18){
					callAjax("initSearch",params2);
				}else{
					ons.notification.alert({
						message: 'Así de Rápido te recuerda, la venta de cigarrillos está prohibida para menores de edad.',
						title: 'Así de Rápido',
				...
				ons.notification.alert({
					message: 'Registrate o ingresa a tu cuenta para acceder a esta sección completa.',
					title: 'Así de Rápido',
					buttonLabel : getTrans('Ok','ok'),
					callback: function(){
						callAjax("initSearch",params2);
    		...
    		//toastMsg( 'Recuerda, la venta de cigarrillos está prohibida para menores de edad' );
    	}else{
          callAjax("initSearch",params2);
    	}
    }	
...
function callAjax(action,params)
{
    ...
    console.log(ajax_url+"/"+action+"?"+params);
	...
	success: function (data) {	  
		...
		if (data.code==1){
			switch (action)
			{
				...
				setStorage("cuisine",data.details.delivery_fee);
				...
				/*VOLVER*/
				case "loadDeliveryOpt":
				$("#page-delivery-opt-page .wrapper").show();				
				$(".delivery-opt-footer").show();
				$("#page-delivery-opt-page .frm-delivery-options").show();
				...
				displayDeliveryOpt(data.details);
				...
				if(data.details.validation_msg == ''){
					...
					if ( data.details.delivery_options){
						...
						$(".delivery_time").val( data.details.delivery_time );
						
						$(".cart_delivery_date_value").html( data.details.delivery_options.default_value_date.date_pretty );
						$(".cart_delivery_time_value").html( data.details.delivery_options.default_value_time.time_pretty );
					}
				}else{
					displayMerchantLogoOnly(data.details,'page-cart');
				  toastMsg(data.details.validation_msg);
					
					$("#cart_to_shipping").hide();
				    ...
				    if ( data.details=="shipping"){		
					    ...
					    var options = {
					      ...
					      onTransitionEnd: function() { 						      	  
					      	  ...
					      	  var cart_params=JSON.stringify(cart);
					      	  var params="merchant_id="+ getStorage("merchant_id");
					      	  params+="&update_cart="+ encodeURIComponent(cart_params);
					      	  params+="&client_token="+ getStorage("client_token");
				...
				case "signup":
				    ...
				    /* temporal */
				    var id_card = (typeof data.details.id_card !== 'undefined' || data.details.id_card != null || data.details.id_card != "" || data.details.id_card != 'null')?data.details.id_card:'';
				    var birth = (typeof data.details.birthdate !== 'undefined' || data.details.birthdate != null || data.details.birthdate != "" || data.details.birthdate != 'null')?data.details.birthdate:'';
				    setStorage("customer_id_card", id_card );
	      		  	setStorage("customer_birthday", birth );
				   ...
				   // mobile verification
					    ...
					    //pushPage("SignupVerification.html", options);
					    menu.setMainPage('SignupVerification.html', {closeMenu: true});
					    //fornez
					// email verification
				   ...
				   /*$(".client_id_sandbox").val( data.details.paypal_credentials.client_id_sandbox );
			        ...
			        translateValidationForm();
			   	   displayPaymentOptions(data);
							...
							$("#emailPay").val(data.details.client_email_address);
							$("#emailPayZ").val(data.details.client_email_address);
							$("#emailPayC").val(data.details.client_email_address);
							$("#phonePayC").val(data.details.client_contact_phone);
							$("#emailMobilePay").val(data.details.client_email_address);
				...
				case "getProfile":
				  $("#cf-salute").text( '¡Hola '+data.details.first_name+'!' );
				  $(".first_name").val( data.details.first_name );
				  $(".last_name").val( data.details.last_name );
				  $(".identification_card").val( data.details.id_card );
				  $(".birthday").val( data.details.birthdate );
				  $(".email_address").val( data.details.email_address );
				  $(".contact_phone").val( data.details.contact_phone );

				  if($(".identification_card").val() != ''){
				  	$(".identification_card").prop('disabled',true);
				  }
			    ...
			    case "initSearch":
			       ...
			       if(isLogin()){
				       ...
				       var options = {     
						   ...
						   var reminder_val = parseInt(data.details.review_reminder);
							//console.log('reminder: '+reminder_val);
							if(reminder_val>0){
							    ons.notification.alert({
							        message: 'Recuerda dejar tu calificación, con una breve review de tu compra anterior.',
							        title: 'Así de Rápido',
							        buttonLabel : getTrans('Ok','ok')
							    });
							} 				   
			...
			{					
				...
				/* V 1.10 */
				case "loadDeliveryOpt":				
				  displayMerchantLogo(data.details,'page-delivery-opt-page');
				  //onsenAlert(data.msg);
				  toastMsg(data.msg);
				  
				  $("#page-delivery-opt-page .wrapper").hide();				
				  $("#page-delivery-opt-page .frm-delivery-options").hide();	
				  $(".delivery-opt-footer").hide();
				  showCartNosOrder();
				  break;
...
{	
    ...
    $.each( data, function( key, val ) {     
    	 ...
    	 htm+='<ons-row class="row stic-pad-top" >';    	 
    	     ...
    	     htm+='<ons-col class="stic-align col-description border" style="margin-left:10px" width="58%">';//Datos de establecimientos
    	           htm+='<div>';
	    	           ...
	    	           // 	   htm+='<p class="stic-info stic-space">|</p>';
	    	           // }
	    	           
	    	           //if(val.service!=3){
	    	           // if(val.service==1 || val.service==2 || val.service==4 || val.service==5 ){
	    	           // 	   if(!empty(val.delivery_estimation)){
	    	           // 	   	  htm+='<ons-icon class="stic-icon" icon="ion-android-time"></ons-icon>';
	    	           // 	      htm+='<p class="stic-info">'+val.delivery_estimation+'</p>';	    	           	   
	    	           // 	      htm+='<p class="stic-info stic-space">|</p>';	    	           	   
	    	           // 	   }
	    	           ...
	    	           // 	  htm+='<span class="notification stic-sponsored sponsored">'+ getTrans("Sponsored",'sponsored') +'</span>';
	    	           // }
...
function displayMerchantLogo2(logo,total,total_usd,page_id)
{
	if(!empty(logo)){
	    $("#"+ page_id +" .logo-wrap").html('<img src="'+logo+'" />')		
	}
	if (!empty(total)){
		$("#"+ page_id +" .total-amount").html(total); //este hay
	}

	if (!empty(total_usd)){
		$("#"+ page_id +" .total-amount-usd").html('<i class="fa fa-usd" aria-hidden="true"></i> '+total_usd+' <small>(Aprox)</small>'); //este hay
	}
...
/* V 1.10 */
function displayMerchantLogoOnly(data,page_id)
{
	if(!empty(data.merchant_info)){
		$("#"+ page_id +" .logo-wrap").html('<img src="'+data.merchant_info.logo+'" />')		
	}
...
function displayMerchantLogoAndName(logo,page_id)
{
	if(!empty(logo)){
	    $("#"+ page_id +" .logo-wrap").html('<img src="'+logo+'" />')		
	}
	...
	$( document ).on( "click", ".transaction_type", function() {
		...
		setStorage('transaction_type',transaction_type);
		  ...
		  if ( !empty($(".delivery_time").val()) ){
      	  ...
      	  callAjax("loadDeliveryOpt","merchant_id="+ getStorage('merchant_id')+"&search_address=" + 
      	  encodeURIComponent(getStorage("search_address")) + "&cart="+cart_params +"&transaction_type=" +
	...
	$( document ).on( "click", ".payment_list", function() {		
		...
		switch( $(this).val() )
		{
			...
			case "pyp":			  			  
			  ...
			  $(".make-order-wrapper").hide();
			  $(".make-order-wrapper_m").hide();
			...
			if ( transaction_type =="delivery" || transaction_type =="pickup" || transaction_type =="dinein" ) {
				...
				$("#report_payment").show();
				$("#amountPay").val(getStorage("order_total_raw"));
				$("#amountPay_m").val(getStorage("order_total_raw"));
			}
			$(".make-order-wrapper").hide();
			$(".make-order-wrapper_m").hide();
			$(".payon-delivery-wrapper").hide();
			$(".zelle-order-wrapper").hide();
			$(".cash-order-wrapper").hide();
			$(".mobilepay-change-wrapper").hide();
			break;
			//probando añadir mis pagos
			case "atz":
			if (paypal_card_fee>0){
				$(".total-amount").html( getStorage("order_total"));
			}
			...
			//console.log('Pago con Zelle');--**.**
				...
				$(".cash-order-wrapper").hide();
				$(".mobilepay-change-wrapper").hide();
				$(".zelle-order-wrapper").show();
				$("#report_payment").show();
				$("#amountZelle").inputFilter(function(value) {return /^-?\d*[.,]?\d{0,2}$/.test(value); });
				$("#amountZelle").val(getStorage("order_usd_total_raw"));
				$("#amountZelleH").val(getStorage("order_usd_total_raw"));
			break;

			case "usc":
			//console.log('Pago Cash USD');
			$(".total-amount").html( getStorage("order_total"));
				...
				$(".zelle-order-wrapper").hide();
				$(".mobilepay-change-wrapper").hide();
				$(".cash-order-wrapper").show();
				$("#report_payment").show();
				$("#amountCashH").val(getStorage("order_usd_total_raw"));
			break;
			...
			$(".total-amount").html( getStorage("order_total"));
				...
				$(".zelle-order-wrapper").hide();
				$(".cash-order-wrapper").hide();
				$(".mobilepay-change-wrapper").show();
				$("#report_payment").show();
				$("#phoneMobilePay").inputFilter(function(value) {return /^\d*$/.test(value); });
				$("#idcardMobilePay").inputFilter(function(value) {return /^\d*$/.test(value); });
				$("#amountMobilePay").inputFilter(function(value) {return /^-?\d*[.,]?\d{0,2}$/.test(value); });
				$("#amountMobilePay").val(getStorage("order_total_raw"));
			break;
	    ...
	    onSuccess : function() {  
	       ...
	       var options = {
		      ...
		      onTransitionEnd: function() { 						      	  
		      	  ...
		      	  params+="&identification_card="+ $(".identification_card").val();
		      	  params+="&device_id="+ getStorage("device_id");
		      	  params+="&tips_percentage=" + getStorage("tips_percentage");
		      	  
		      	  //params+="&cart_subtotal="+ $(".cart_subtotal").val();
		      	  		      	  
		      	  callAjax("loadCart",params+extra_params);
		      } 
		    };   
		    sNavigator.pushPage("cart.html", options);		       
	       return false;
...
/* V 1.10 */
function displayDeliveryOpt(data)
{	
	// display merchant logo
	displayMerchantLogo2(data.merchant_info.logo,'','page-delivery-opt-page');
 ...
 //    if (!empty(data.cart)){
	...
	htm+='<ons-list-header class="trn" data-trn-key="delivery_options" style="display: none;">Delivery Options</ons-list-header>';
	...
	createElement('delivery-opt-item',htm);
...
{	
    ...
    setStorage("usd_total",data.cart.grand_total.amount_usd);
    
    setStorage("order_total_raw",data.cart.grand_total.amount);
    setStorage("order_usd_total_raw", data.cart.grand_total.amount_usd);
    console.log(data.cart.grand_total.amount_usd);
    ...
    if(!empty(data.cart.cargos)){
       setStorage("cargos", data.cart.cargos.amount ); //i know mucho
    }
	...
	if (!empty(data.cart)){
		...
		if(!empty(data.cart.grand_total.amount_usd)){
		   $(".total-amount-usd").html('<i class="fa fa-usd" aria-hidden="true"></i> '+data.cart.grand_total.amount_usd+' <small>(Aprox)</small>');
		}
		...
		if (!empty(data.cart.discount)){			
			...
			htm+=tplCartRow(getTrans('Discount','discount'), '('+data.cart.discount.amount_pretty+')' ,'price-normal' );
		}				
		...
		htm+=tplCartRow( getTrans('Cargo por uso de la aplicación','Cargo por uso de la aplicación'), data.cart.cargos.amount_pretty, 'price-normal');
		if (!empty(data.cart.tax)){
...
function showDeliveryOptionsPage() {
	...
	sNavigator.pushPage("delivery-opt-page.html", options);
	// if ( cart.length<1){
	// 	onsenAlert( getTrans("Your cart is empty",'your_cart_is_empty') );
	// 	return;
	// }
	
	// setTrackView("delivery-opt-page");
	
 //    callAjax("deliveryOptionPage","merchant_id="+ getStorage('merchant_id')+"&search_address=" + 
...
{	
 ...
 // codigo para mostar la pagina de reportar pago
function ReportarPago()
{
	if (isLogin()){
		menu.setMainPage('reportarpago.html', {closeMenu: true});
	} else {
		menu.setMainPage('prelogin.html', {closeMenu: true})
	}
}
...
function clientRegistration()
{
	$.validate({ 	
	    ...
	    onSuccess : function() {     	
	      ...
	      setStorage("customer_id_card",  $(".identification_card").val()  );
	      setStorage("customer_birthday",  $(".birthday").val()  );
...
{	
	...
	var options = {
      ...
      onTransitionEnd: function() { 						      	  
	      ...
	      callAjax("getPaymentOptions",params+cart_params);
      } 
    };
    sNavigator.pushPage("paymentOption.html", options);
...
function displayPaymentOptions(data)
{
	...
	if (data.details.pay_on_delivery_flag==1){
		$.each( $(data.details.pay_on_delivery_list) , function( key, val ) { 			
		    //dump(val);
		    //muestro la pantalla de pagar con Mis Pagos Mercantil
		    /*htm+='<p style="font-weight: bold !important;text-align: center !important;background-color: #3467bb;padding: 20px;margin: 0px -2px 25px -2px;color: white;">Paso 1: Pagar con Mis Pagos Mercantil</p>';
	    	htm+='<a href="https://www.portaldepagosmercantil.com/" onclick="window.open("https://www.portaldepagosmercantil.com/", "_blank");"><img src="https://www.asiderapido.com/images/mispagos.jpg" width="250px" class="center-imagenes"></a>';
	    	htm+='<p style="font-size:14px !important;">Si deseas pagar con tarjeta de credito Visa o Mastercard, o con tu tarjeta de debidto Mercantil puedes hacerlo a través del portal de pagos del Banco Mercantil.</p>';
		    htm+='<p style="font-weight: bold !important;text-align: center !important;background-color: #3467bb;padding: 20px;margin: 0px -2px 25px -2px;color: white;">Paso 2: Reportar pago</p>';		    
		    htm+='<p style="font-size:16px !important;">Haznos saber que ya hiciste la transferencia reportando tu pago en nuestra plataforma.</p>';
		    htm+='';
		    htm+='<p style="font-weight: bold !important;text-align: center !important;background-color: #3467bb;padding: 20px;margin: 25px -2px -2px -2px;color: white;">Paso 3: Realizar pedido</p>';
	    	htm+='<p style="font-size:14px !important;">¿Ya realizaste el pago?</p>';*/
	    	...
	    	htm+='</span>';
	    	//htm+='<p style="font-size:16px !important;">Si ya está todo listo. !Vamos a realizar el pedido!</p>';
			//htm+='<button class="button green-btn button--large marg20 trn" onclick="InitPlaceOrder();" data-trn-key="place_order" id="txtAge">¡Pidemelo!</button>';
...
/* INICIO REPORTE DE PAGO (FUNCIÓN) */
...
/*NUEVO*/
function preSignup(){
	if($('.identification_card').val().length>7){
		signup();	
	}else{
		toastMsg("Verifique su cédula");	
	}
	return false;
}
...
function displayOrderHistoryDetails(data)
{
	...
	var html='<button class="button green-btn button--large marg20 trn" onclick="reOrder('+data.order_id+');" data-trn-key="click_here_to_reorder" >';
...
{	
	...
	/* NUEVO */
	//toastMsg("Temporalmente estamos cubriendo sólo en algunas zonas de Valencia, estamos trabajando para ampliar nuestros servicios, ofrecemos disculpas por las molestias causadas. Atentos a nuestras redes sociales para más información. Síguenos en @asiderapido")
...
function itemNotAvailable(options)
{
	...
	{
		case 1:
		toastMsg( getTrans("no disponible",'item_not_available') );
		break;
...
function goBackSearchByAddress()
{
	search_mode = 'address';
	var ss = $("#ss").val();
	/*clear all storage*/ 
    setStorage("search_address",ss);
    clearAllStorage();
    callAjax("initSearch","search_mode=" + search_mode );			   
}
...
/* FUNCIÓN PROCESSOR BACKEND*/

function initReportPayment() {
	...
	if ( $('.payment_list:checked').length > 0){
		...
		//dump(selected_payment);
		if ( selected_payment=="cod"){
			var number  = $("#numberPay");
	    var bank    = $("#bankPay");
	    var amount  = $("#amountPay");
	    var email   = $("#emailPay");
	    var res = $("#resLb");

	    if(number.val() == "" || number.val() == null || number.val() == undefined) {
			    cleanBorder();
			    wrongData(number.attr('name'));
			    wrongMessages(res.attr('id'),"number");
			}
			else if (bank.val() == "" || bank.val() == null || bank.val() == undefined) {
			    cleanBorder();
			    wrongData(bank.attr('name'));
			    wrongMessages(res.attr('id'),"bank");
			}
			else if(amount.val() == "" || amount.val() == null || amount.val() == undefined) {
			    cleanBorder();
			    wrongData(amount.attr('name'));
			    wrongMessages(res.attr('id'),"amount");
			}
			else if(email.val() == "" || email.val() == null || email.val() == undefined) {
			    cleanBorder();
			    wrongData(email.attr('name'));
			    wrongMessages(res.attr('id'),"email");
			}
			else {
			    ...
			    if (caract.test(email.val()) == false) {
			        cleanBorder();
			        wrongData(email.attr('name'));
			        wrongMessages(res.attr('id'),"wrongEmail");
			    }
			    else {
			        cleanBorder();
			        ...
			        $.ajax({
			        	data: JSON.stringify(_dJson),
			        	type: 'post',
			        	dataType: 'json',
			        	url: 'https://www.asiderapido.com/procesador/server/sendMailMobile.php'
			        })
			        .done(function(data){
			        	if(data.success == false){
			        		document.getElementById(res.attr('id')).innerHTML = data.messages;
			        	}else{
			        		document.getElementById(res.attr('id')).innerHTML = data.messages;
			        		if ( selected_payment=="cod" ){
			        			$('#report_payment').hide();
			        			$('#make-order-wrapper').show();
			        			$('#make-order-wrapper .button').focus();
			        		} else if ( selected_payment=="pyr" ){
			        			$('#report_payment_m').hide();
			        			$('#make-order-wrapper_m').show();
			        			$('#make-order-wrapper .button').focus();
			        		}
			            	hideAllModal();
			        	}
			        ...
			        .fail(function(data){
			        	document.getElementById(res.attr('id')).innerHTML = 'La solicitud ha fallado '+data.messages;
			        });            
			    }  
			}

		}else if ( selected_payment=="trz"){
			console.log('trz aki');
			
			var holderName  = $("#nameZelle");
	    var emailZ    = $("#emailZelle");
	    var amount  = $("#amountZelle");
	    var amountH  = $("#amountZelleH");
	    var email   = $("#emailPayZ");
	    var res = $("#resLb");

			if(holderName.val() == "" || holderName.val() == null || holderName.val() == undefined) {
			    cleanBorderZelle();
			    wrongData(holderName.attr('name'));
			    wrongMessages(res.attr('id'),"nameZelle");
			}else if(emailZ.val() == "" || emailZ.val() == null || emailZ.val() == undefined) {
			    cleanBorderZelle();
			    wrongData(emailZ.attr('name'));
			    wrongMessages(res.attr('id'),"emailZelle");
			}else if(amount.val() == "" || amount.val() == null || amount.val() == undefined) {
			    cleanBorderZelle();
			    wrongData(amount.attr('name'));
			    wrongMessages(res.attr('id'),"amountZelle");
			}else if(Number.parseFloat(amount.val().replace(',','.')) < Number.parseFloat(amountH.val().replace(',','.'))){
					cleanBorderZelle();
			    wrongData(amount.attr('name'));
			    wrongMessages(res.attr('id'),"amountZelleH");
			}else{
				var caract = new RegExp(/^[-\w.%+]{1,64}@(?:[A-Z0-9-]{1,63}\.){1,125}[A-Z]{2,63}$/i);

				if (caract.test(emailZ.val()) == false) {
					cleanBorderZelle();
					wrongData(emailZ.attr('name'));
					wrongMessages(res.attr('id'),"wrongEmail");
				}else{
	    ...
	    var amountH = $("#amountCashH");
	    var email = $("#emailPayC");
	    var phone = $('#phonePayC');
	    var change = $("#changePay");
	    var res = $("#resLb");

			if(change.val() == "" || change.val() == null || change.val() == undefined || Number.parseFloat(change.val().replace(',','.')) < 0) {
			    wrongMessages(res.attr('id'),"changeCash");
			}else{
		...
		}else if ( selected_payment=="mpt"){
			console.log('mpt aki');
			
			var phoneM = $("#phoneMobilePay");
	    var idcardM = $("#idcardMobilePay");
	    var bankM = $("#bankMobilePay");
	    var amountM = $("#amountMobilePay");
	    var emailM = $("#emailMobilePay");
	    var res = $("#resLb");

			if(phoneM.val() == "" || phoneM.val() == null || phoneM.val() == undefined) {
			    cleanBorderMobilePay();
			    wrongData(phoneM.attr('name'));
			    wrongMessages(res.attr('id'),"phoneM");
			}else if(idcardM.val() == "" || idcardM.val() == null || idcardM.val() == undefined) {
			    cleanBorderMobilePay();
			    wrongData(idcardM.attr('name'));
			    wrongMessages(res.attr('id'),"idcardM");
			}else if(bankM.val() == "" || bankM.val() == null || bankM.val() == undefined) {
			    cleanBorderMobilePay();
			    wrongData(bankM.attr('name'));
			    wrongMessages(res.attr('id'),"bankM");
			}else if(amountM.val() == "" || amountM.val() == null || amountM.val() == undefined) {
			    cleanBorderMobilePay();
			    wrongData(amountM.attr('name'));
			    wrongMessages(res.attr('id'),"amountM");
			}else{
...
/* FUNCIÓN COLIFAVOR */
function initColifavor() {

	var order = $(".order-txta");
	var deliveryA = $(".delivery-add-txta");
	var pickupA = $(".pickup-addr-txta");
	var orderVal = $(".order-value");

	var region = $("#cf_region").find(":selected").text();
	var sector = $("#cf_sector").find(":selected").text();

	var delivery_date = $(".cf_delivery_date");
	var delivery_time = $(".cf_delivery_time");
	...
	if(order.val() == "" || order.val() == null || order.val() == undefined) {
		cleanBorderCF();
		wrongData(order.attr('name'));
		wrongMessages(res.attr('id'),"order");
	}else if(deliveryA.val() == "" || deliveryA.val() == null || deliveryA.val() == undefined) {
		cleanBorderCF();
		wrongData(deliveryA.attr('name'));
		wrongMessages(res.attr('id'),"deliveryA");
	}else if(pickupA.val() == "" || pickupA.val() == null || pickupA.val() == undefined) {
		cleanBorderCF();
		wrongData(pickupA.attr('name'));
		wrongMessages(res.attr('id'),"pickupA");
	}else if(orderVal.val() == "" || orderVal.val() == null || orderVal.val() == undefined) {
		cleanBorderCF();
		wrongData(orderVal.attr('name'));
		wrongMessages(res.attr('id'),"orderVal");
	}
	else {
		cleanBorderCF();
	    ...
	    var _dJson = {
	    	"payType":'cf-serv',
	        "order":order.val(),
	        "delivery-add":deliveryA.val(),
	        "region":region,
					"sector":sector,
	        "pickup-add":pickupA.val(),
	        "order-val":orderVal.val(),
...
/* FUNCIÓN COLITAXI */
function initColitaxi() {

	var destination = $(".destination-txta");
	var actualLocation = $(".actual-location-txta");
	var passengerName = $(".passenger_name");
	var passengerPhone = $(".passenger_phone");

	var region = $("#ct_region").find(":selected").text();
	var sector = $("#ct_sector").find(":selected").text();

	var travel_date = $(".ct_travel_date");
	var travel_time = $(".ct_travel_time");
	...
	if(destination.val() == "" || destination.val() == null || destination.val() == undefined) {
		cleanBorderTaxi();
		wrongData(destination.attr('name'));
		wrongMessages(res.attr('id'),"destination");
	}else if(actualLocation.val() == "" || actualLocation.val() == null || actualLocation.val() == undefined) {
		cleanBorderTaxi();
		wrongData(actualLocation.attr('name'));
		wrongMessages(res.attr('id'),"actualLocation");
	}else if(passengerName.val() == "" || passengerName.val() == null || passengerName.val() == undefined) {
		cleanBorderTaxi();
		wrongData(passengerName.attr('name'));
		wrongMessages(res.attr('id'),"passengerName");
	}else if(passengerPhone.val() == "" || passengerPhone.val() == null || passengerPhone.val() == undefined) {
		cleanBorderTaxi();
		wrongData(passengerPhone.attr('name'));
		wrongMessages(res.attr('id'),"passengerPhone");
	}
	else {
		cleanBorderTaxi();
	    ...
	    var _dJson = {
	        ...
	        "actual-location":actualLocation.val(),
	        "region":region,
					"sector":sector,
	        "passenger-name":passengerName.val(),
	        "passenger-phone":passengerPhone.val(),
...
function wrongData(value) {
    document.getElementById(value).style.borderBottom = "1px solid #ff1212";
}

function cleanBorder() {
    document.getElementById("numberPay").style.borderBottom = "1px solid #333";
    document.getElementById("bankPay").style.borderBottom = "1px solid #333";
    document.getElementById("amountPay").style.borderBottom = "1px solid #333";
    document.getElementById("emailPay").style.borderBottom = "1px solid #333";

    document.getElementById("numberPay_m").style.borderBottom = "1px solid #333";
    document.getElementById("bankPay_m").style.borderBottom = "1px solid #333";
    document.getElementById("amountPay_m").style.borderBottom = "1px solid #333";
    document.getElementById("emailPay_m").style.borderBottom = "1px solid #333";  
}

function cleanBorderZelle() {
    document.getElementById("nameZelle").style.borderBottom = "1px solid #333";
    document.getElementById("emailZelle").style.borderBottom = "1px solid #333";
    document.getElementById("amountZelle").style.borderBottom = "1px solid #333";   
}

function cleanBorderMobilePay() {
    document.getElementById("phoneMobilePay").style.borderBottom = "1px solid #333";
    document.getElementById("idcardMobilePay").style.borderBottom = "1px solid #333";
    document.getElementById("bankMobilePay").style.borderBottom = "1px solid #333";
    document.getElementById("amountMobilePay").style.borderBottom = "1px solid #333";
}

function cleanBorderCF() {
	document.getElementById("order-txta").style.borderBottom = "1px solid #333";
	document.getElementById("delivery-add-txta").style.borderBottom = "1px solid #333";
	document.getElementById("pickup-addr-txta").style.borderBottom = "1px solid #333";
	document.getElementById("order-value").style.borderBottom = "1px solid #333";
}

function cleanBorderTaxi() {
	document.getElementById("destination-txta").style.borderBottom = "1px solid #333";
	document.getElementById("actual-location-txta").style.borderBottom = "1px solid #333";
	document.getElementById("passenger_name").style.borderBottom = "1px solid #333";
	document.getElementById("passenger_phone").style.borderBottom = "1px solid #333";
}

function wrongMessages(l,m) {
	//console.log(l);
    switch(m) {
        case "number":
            document.getElementById(l).innerHTML = "Debe ingresar el numero de transferencia.";
        break;    
        
        case "bank":
            document.getElementById(l).innerHTML = "Debe seleccionar el banco.";
        break;   

        case "amount":
            document.getElementById(l).innerHTML = "Debe ingresar el monto de la transferencia.";
        break;   

        case "email":
            document.getElementById(l).innerHTML = "Debe ingresar el correo que tiene registrado con nosotros.";
        break;   

        case "wrongEmail":
            document.getElementById(l).innerHTML = "El correo que ha ingresado no es valido.";
        break;

        case "order":
            document.getElementById(l).innerHTML = "Debe ingresar un pedido para procesar.";
        break;

        case "deliveryA":
            document.getElementById(l).innerHTML = "Debe proporcionar una dirección de envío.";
        break;

        case "pickupA":
            document.getElementById(l).innerHTML = "Debe ingresar una dirección de retiro.";
        break;

        case "orderVal":
            document.getElementById(l).innerHTML = "Debe ingresar un valor aproximado del total del pedido.";
        break;

        case "destination":
            document.getElementById(l).innerHTML = "Debe ingresar una dirección de destino.";
        break;

        case "actualLocation":
            document.getElementById(l).innerHTML = "Debe proporcionar su dirección actual.";
        break;

        case "passengerName":
            document.getElementById(l).innerHTML = "Debe ingresar el nombre de quien contrata el servicio.";
        break;

        case "passengerPhone":
            document.getElementById(l).innerHTML = "Introduzca un número de contacto.";
        break;

        case "nameZelle":
            document.getElementById(l).innerHTML = "Introduzca el nombre del titular que transfiere Zelle.";
        break;

        case "emailZelle":
            document.getElementById(l).innerHTML = "Introduzca el email del titular que transfiere Zelle.";
        break;

        case "amountZelle":
            document.getElementById(l).innerHTML = "Introduzca el monto Zelle transferido.";
        break;

        case "amountZelleH":
            document.getElementById(l).innerHTML = "El monto transferido no puede ser menor al monto de la factura.";
        break;

        case "changeCash":
            document.getElementById(l).innerHTML = "Utilice los contadores de arriba segun su disponibilidad de efectivo, hasta alcanzar el monto de la factura.";
        break;

        case "phoneM":
            document.getElementById(l).innerHTML = "Debe ingresar el teléfono asociado al Pago Móvil desde donde emitió el pago.";
        break;

        case "idcardM":
            document.getElementById(l).innerHTML = "Ingrese la cédula asociada al Pago Móvil desde donde emitió el pago.";
        break;

        case "bankM":
            document.getElementById(l).innerHTML = "Seleccione la entidad bancaria de su Pago Móvil.";
        break;

        case "amountM":
            document.getElementById(l).innerHTML = "Ingrese el monto transferido desde Pago Móvil.";
        break;
    }
}
...
/*FAVOR NUEVO*/
function showColifavor()
{
	if (isLogin()){
		ons.notification.alert({
			message: 'Podemos llevarte cualquier cosa disponible en tu ciudad, siempre y cuando no sea, de un peso mayor a los 10kg y de dimensiones mayores a 60x50x50cm. El servicio estará disponibe entre las 10:00am y las 10:00pm',
			title: 'Limites de Entrega',
			buttonLabel : getTrans('Ok','ok')
		});
		menu.setMainPage('colifavor.html', {closeMenu: true});
	} else {
		menu.setMainPage('login.html', {closeMenu: true})
	}
}

function showServiciosP()
{
	menu.setMainPage('services.html');
	var modal = $('#mdUbic');
	modal.hide();
}

/*TAXI*/
function showColitaxi()
{
	if (isLogin()){
		ons.notification.alert({
			message: 'Por los momentos no contamos con inmediatez en el servicio. Tome en cuenta que la atención, será canalizada lo antes posible dentro de los próximos 15 minutos y estará disponible entre las 10:00am y las 10:00pm. ',
			title: 'Colitaxi',
			buttonLabel : getTrans('Ok','ok')
		});
		//menu.setMainPage('colitaxi.html', {closeMenu: true});
	} else {
		menu.setMainPage('login.html', {closeMenu: true})
	}
}

var incrementValue = function (id) {
  var inp = document.getElementById(id);
  //console.log(inp);
  var actVal = parseInt(inp.value,10);
  //console.log(actVal);
...
var decrementValue = function (id) {
  var inp = document.getElementById(id);
  //console.log(inp);
  var actVal = parseInt(inp.value,10);
  //console.log(actVal);
...
var reportChangePay = function(){
	var cartVal = $('#amountCashH').val().replace(',','.');
	var rpt = $('#changePayment');
	var changeIn = $('#changePay');
	var rsp = $('#warningRsp');

	var d100 = parseInt($('#usd100').val())*100;
	var d50 = parseInt($('#usd50').val())*50;
	var d20 = parseInt($('#usd20').val())*20;
	var d10 = parseInt($('#usd10').val())*10;
	var d5 = parseInt($('#usd5').val())*5;
	var d1 = parseInt($('#usd1').val());

	var rchange = d100 + d50 + d20 + d10 + d5 + d1 - parseFloat(cartVal);

	if(rchange%1 > 0.75) {
		//console.log(Math.floor(rchange));
		rchange = Math.ceil(rchange);
	}else{
		//console.log(Math.ceil(rchange));
		rchange = Math.floor(rchange);
	}

	if(rchange < 0 || rchange > 20){
		rpt.css('color','red');
	}else{
		rpt.css('color','darkturquoise');
	}

	rpt.html(rchange.toFixed(2));
	changeIn.val(rchange.toFixed(2));
	if (rchange > 20) {
		rsp.html('El monto de su vuelto, supera el límite establecido. Revise nuevamente la configuración de monedas que reportó arriba o busque otras denominaciones de monedas para pagar su pedido.');
	}else{
		rsp.html('');
	}
...
var loadSectors = function(id){

	switch (id.substr(0,2)) {

		case 'ct':
			var rgn = $('#'+id).find(":selected").val();

			if(rgn == ''){
				$('#ct_sector').find('option').remove().end();
				$('#ct_sector').prop('disabled',true);
				$('#ct_sector').hide();
			}else if(rgn == 'cval'){
				$('#ct_sector').find('option').remove().end();
				$('#ct_sector').append('<option value=""> Selecciona el sector </option><option value="el_solar"> El Solar  </option><option value="el_bosque"> El Bosque </option><option value="vill_laguna"> Villas laguna Club </option><option value="vill_lagunante"> Villas laguna Norte </option><option value="col_guataparo"> Colinas de Guataparo </option><option value="el_mirador"> El Mirador </option><option value="pque_mirador"> Parque Mirador </option><option value="agua_blancar"> Agua Blanca </option><option value="valles_camoruco"> Valles de Camoruco </option><option value="prebo_ii"> Prebo 2 </option><option value="prebo_iii"> Prebo 3 </option><option value="altos_prebo"> Altos de Prebo</option><option value="trigal_nte"> Trigal Norte </option><option value="trigal_ctro"> Trigal Centro </option><option value="trigal_sur"> Trigal Sur </option><option value="la_vina"> La Viña </option><option value="lomas_vina"> Lomas de la Viña </option><option value="el_recreo"> El Recreo </option><option value="los_chaguaramos"> Los chaguaramos </option><option value="los_mangos"> Los Mangos </option><option value="santa_cecilia"> Santa Cecilia </option><option value="camoru"> Camoruco </option><option value="viejo_camoru">  El Viejo Camoruco </option><option value="la_alegria"> La Alegria </option><option value="sanj_tarbes"> San José de Tarbes </option><option value="los_nisperos"> Los Nisperos </option><option value="terraza_nisperos"> Terraza de los Nisperos </option><option value="l_kerdell"> Kerdell </option><option value="alts_parral"> Altos del Parral </option><option value="cuatro_av"> 4 Avenidas </option><option value="bolivar_val"> Av Bolívar - Valencia </option><option value="cedeno_av"> Av Cedeño </option><option value="parral"> El Parral </option><option value="guaparo"> Guaparo </option><option value="guataparo"> Guataparo </option><option value="chimeneas"> Las Chimeneas </option><option value="lomas_este"> Lomas del Este </option><option value="colorados"> Los Colorados </option><option value="prebo"> Prebo </option><option value="trigal"> Trigal / Trigaleña </option><option value="vinedo"> Viña / Viñedo </option>');
				$('#ct_sector').show();
				$('#ct_sector').prop('disabled',false);
			}else if(rgn == 'cnagua') {
				$('#ct_sector').find('option').remove().end();
				$('#ct_sector').append('<option value=""> Selecciona el sector </option><option value="la_campinaii"> La Campiña II </option><option value="la_campinaiii"> La Campiña III </option><option value="la_campina"> La Campiña  </option><option value="los_guayabitos"> Los Guayabitos </option><option value="la_begona"> La Begoña </option><option value="la_rotafe"> Rotafe </option><option value="la_ucbarbula"> Ciudad Universitaria Bárbula </option><option value="bolivar_nag"> Av Bolívar - Naguanagua </option><option value="rincon"> El Rincón </option><option value="granja"> La Granja </option><option value="caracaros"> Los Caracaros </option><option value="guayabitos"> Los Guayabitos </option><option value="manongo"> Mañongo </option><option value="naguanagua"> Naguanagua </option><option value="paramacay"> Resd Militares Paramacay </option><option value="rio_sil"> Rio Sil </option><option value="tazajal"> Tazajal </option>');
				$('#ct_sector').show();
				$('#ct_sector').prop('disabled',false);
			}else if(rgn == 'csdiego'){
				$('#ct_sector').find('option').remove().end();
				$('#ct_sector').append('<option value=""> Selecciona el sector </option><option value="el_remanzo"> El Remanzo </option><option value="vi_bahiasdiegollas"> Villas Bahía de San Diego </option><option value="el_remanzo"> El Remanzo </option><option value="pque_serino"> Parqueserino </option><option value="lmas_esmeralda"> Lomas de la Esmeralda </option><option value="yuma_i"> Yuma I </option><option value="yuma_ii"> Yuma II </option><option value="yuma_iii"> Yuma III </option><option value="valle_oro"> Valle de Oro </option><option value="los_andes"> Los Andes</option><option value="terraza_sndiego"> Terrazas de San Diego </option><option value="la_gaviota"> La Gaviota </option><option value="sta_ana"> Santa Ana </option><option value="sd_variante"> Variante </option><option value="sd_tulipanes"> Tulipanes </option><option value="sd_remanso"> Remanso </option><option value="sd_monteserino"> Monteserino </option><option value="sd_bosqueserino"> Bosqueserino </option><option value="sd_pueblo"> Pueblo de San Diego </option><option value="sd_macomaco"> Macomaco </option><option value="sd_esmeralda"> La Esmeralda </option><option value="sd_biglow"> Big Low </option><option value="sd_golf"> Campo de Golf </option><option value="sd_ujap"> UJAP (Univ. José Antonio Páez) </option><option value="sd_uam"> UAM (Univ. Arturo Michelena) </option><option value="sd_pasoreal"> Paso Real </option><option value="sd_arales"> Los Arales </option><option value="sd_morro"> El Morro </option><option value="sd_morro2"> Morro II </option><option value="sd_industrial"> Zona Industrial de San Diego </option><option value="sd_cumaca"> La Cumaca </option>');
				$('#ct_sector').show();
				$('#ct_sector').prop('disabled',false);
			}else{
				$('#ct_sector').find('option').remove().end();
		...
		case 'cf':
			...
			if(rgn == ''){
				$('#cf_sector').find('option').remove().end();
				$('#cf_sector').prop('disabled',true);
				$('#cf_sector').hide();
			}else if(rgn == 'cval'){
				$('#cf_sector').find('option').remove().end();
				$('#cf_sector').append('<option value=""> Selecciona el sector </option><option value="el_solar"> El Solar  </option><option value="el_bosque"> El Bosque </option><option value="vill_laguna"> Villas laguna Club </option><option value="vill_lagunante"> Villas laguna Norte </option><option value="col_guataparo"> Colinas de Guataparo </option><option value="el_mirador"> El Mirador </option><option value="pque_mirador"> Parque Mirador </option><option value="agua_blancar"> Agua Blanca </option><option value="valles_camoruco"> Valles de Camoruco </option><option value="prebo_ii"> Prebo 2 </option><option value="prebo_iii"> Prebo 3 </option><option value="altos_prebo"> Altos de Prebo</option><option value="trigal_nte"> Trigal Norte </option><option value="trigal_ctro"> Trigal Centro </option><option value="trigal_sur"> Trigal Sur </option><option value="la_vina"> La Viña </option><option value="lomas_vina"> Lomas de la Viña </option><option value="el_recreo"> El Recreo </option><option value="los_chaguaramos"> Los chaguaramos </option><option value="los_mangos"> Los Mangos </option><option value="santa_cecilia"> Santa Cecilia </option><option value="camoru"> Camoruco </option><option value="viejo_camoru">  El Viejo Camoruco </option><option value="la_alegria"> La Alegria </option><option value="sanj_tarbes"> San José de Tarbes </option><option value="los_nisperos"> Los Nisperos </option><option value="terraza_nisperos"> Terraza de los Nisperos </option><option value="l_kerdell"> Kerdell </option><option value="alts_parral"> Altos del Parral </option><option value="cuatro_av"> 4 Avenidas </option><option value="bolivar_val"> Av Bolívar - Valencia </option><option value="cedeno_av"> Av Cedeño </option><option value="parral"> El Parral </option><option value="guaparo"> Guaparo </option><option value="guataparo"> Guataparo </option><option value="chimeneas"> Las Chimeneas </option><option value="lomas_este"> Lomas del Este </option><option value="colorados"> Los Colorados </option><option value="prebo"> Prebo </option><option value="trigal"> Trigal / Trigaleña </option><option value="vinedo"> Viña / Viñedo </option>');
				$('#cf_sector').show();
				$('#cf_sector').prop('disabled',false);
			}else if(rgn == 'cnagua') {
				$('#cf_sector').find('option').remove().end();
				$('#cf_sector').append('<option value=""> Selecciona el sector </option><option value="la_campinaii"> La Campiña II </option><option value="la_campinaiii"> La Campiña III </option><option value="la_campina"> La Campiña  </option><option value="los_guayabitos"> Los Guayabitos </option><option value="la_begona"> La Begoña </option><option value="la_rotafe"> Rotafe </option><option value="la_ucbarbula"> Ciudad Universitaria Bárbula </option><option value="bolivar_nag"> Av Bolívar - Naguanagua </option><option value="rincon"> El Rincón </option><option value="granja"> La Granja </option><option value="caracaros"> Los Caracaros </option><option value="guayabitos"> Los Guayabitos </option><option value="manongo"> Mañongo </option><option value="naguanagua"> Naguanagua </option><option value="paramacay"> Resd Militares Paramacay </option><option value="rio_sil"> Rio Sil </option><option value="tazajal"> Tazajal </option>');
				$('#cf_sector').show();
		...
		case 'cv':
			...
			if(rgn == ''){
				$('#coverage_sector').find('option').remove().end();
				$('#coverage_sector').prop('disabled',true);
				$('#coverage_sector').hide();
			}else if(rgn == 'cval'){
				$('#coverage_sector').find('option').remove().end();
				$('#coverage_sector').append('<option value=""> Selecciona el sector </option><option value="el_solar"> El Solar  </option><option value="el_bosque"> El Bosque </option><option value="vill_laguna"> Villas laguna Club </option><option value="vill_lagunante"> Villas laguna Norte </option><option value="col_guataparo"> Colinas de Guataparo </option><option value="el_mirador"> El Mirador </option><option value="pque_mirador"> Parque Mirador </option><option value="agua_blancar"> Agua Blanca </option><option value="valles_camoruco"> Valles de Camoruco </option><option value="prebo_ii"> Prebo 2 </option><option value="prebo_iii"> Prebo 3 </option><option value="altos_prebo"> Altos de Prebo</option><option value="trigal_nte"> Trigal Norte </option><option value="trigal_ctro"> Trigal Centro </option><option value="trigal_sur"> Trigal Sur </option><option value="la_vina"> La Viña </option><option value="lomas_vina"> Lomas de la Viña </option><option value="el_recreo"> El Recreo </option><option value="los_chaguaramos"> Los chaguaramos </option><option value="los_mangos"> Los Mangos </option><option value="santa_cecilia"> Santa Cecilia </option><option value="camoru"> Camoruco </option><option value="viejo_camoru">  El Viejo Camoruco </option><option value="la_alegria"> La Alegria </option><option value="sanj_tarbes"> San José de Tarbes </option><option value="los_nisperos"> Los Nisperos </option><option value="terraza_nisperos"> Terraza de los Nisperos </option><option value="l_kerdell"> Kerdell </option><option value="alts_parral"> Altos del Parral </option><option value="cuatro_av"> 4 Avenidas </option><option value="bolivar_val"> Av Bolívar - Valencia </option><option value="cedeno_av"> Av Cedeño </option><option value="parral"> El Parral </option><option value="guaparo"> Guaparo </option><option value="guataparo"> Guataparo </option><option value="chimeneas"> Las Chimeneas </option><option value="lomas_este"> Lomas del Este </option><option value="colorados"> Los Colorados </option><option value="prebo"> Prebo </option><option value="trigal"> Trigal / Trigaleña </option><option value="vinedo"> Viña / Viñedo </option>');
				$('#coverage_sector').show();
				$('#coverage_sector').prop('disabled',false);
			}else if(rgn == 'cnagua') {
				$('#coverage_sector').find('option').remove().end();
				$('#coverage_sector').append('<option value=""> Selecciona el sector </option><option value="la_campinaii"> La Campiña II </option><option value="la_campinaiii"> La Campiña III </option><option value="la_campina"> La Campiña  </option><option value="los_guayabitos"> Los Guayabitos </option><option value="la_begona"> La Begoña </option><option value="la_rotafe"> Rotafe </option><option value="la_ucbarbula"> Ciudad Universitaria Bárbula </option><option value="bolivar_nag"> Av Bolívar - Naguanagua </option><option value="rincon"> El Rincón </option><option value="granja"> La Granja </option><option value="caracaros"> Los Caracaros </option><option value="guayabitos"> Los Guayabitos </option><option value="manongo"> Mañongo </option><option value="naguanagua"> Naguanagua </option><option value="paramacay"> Resd Militares Paramacay </option><option value="rio_sil"> Rio Sil </option><option value="tazajal"> Tazajal </option>');
				$('#coverage_sector').show();
				...
				$('#coverage_sector').find('option').remove().end();
				$('#coverage_sector').append('<option value="other" selected> Otro </option>');
				$('#coverage_sector').show();
				$('#coverage_sector').prop('disabled',false);
			}
...
var showServices = function(){
	...
	animateCSS('.cities_container','bounceOut');
	$('.cities_container').on('animationend webkitAnimationEnd oAnimationEnd',hideCities());
	setTimeout(function(){
		$('.services_container').show();
		animateCSS('.services_container','bounceIn');
	},1000);
}
...
function modalTestShow(){
	...
	var btn2 = document.getElementById('btnBuscar');
	var inputB = document.getElementById('ss_restaurant_name');
...
function modalTestHide(){
	var modal = $('#mdTest');
	modal.hide();
}
/* MODAL UBICACION*/
function modalTestShow2(){
	setTimeout(function(){
		var modalU = $('#mdUbic');
		modalU.show();	
	},300);
}
function modalTestHide2(){
	var modal = $('#mdUbic');
	modal.hide();
}
function modalTestShow3(){
	setTimeout(function(){
		var modalR = $('#mdRegist');
		modalR.show();	
	},300);
}
function modalTestHide3(){
	var modal = $('#mdRegist');
	modal.hide();
}
...
function alertColitaxi(){
	ons.notification.alert({
		message: 'Muy pronto podrás pedir este servicio desde donde te encuentres. Ahora en Valencia moverse será AsídeRápido.',
		title: 'Colitaxi',
		buttonLabel : getTrans('Ok','ok')
	});
}
