...
{	
	...
	{		
		...
		$data=''; $total_records=0; $stmt=''; $location_type = '';
		...
		switch ($search_mode) {
			...
			default: /* SEARCH BY ADDRESS*/
			    ...
			    if($stype=="1"){
			    	...
			    	$ss_street = isset($this->data['ss_street'])?trim($this->data['ss_street']):'';
			    	$stmt="SELECT SQL_CALC_FOUND_ROWS a.*
			    	FROM {{view_merchant}} a
			    	WHERE 
					...
					WHERE 1 = 1
					$and
				 	ORDER BY is_featured DESC, is_sponsored DESC, restaurant_name
					LIMIT $limit_page
					";
			    }
		...
		if (isset($_GET['debug'])){
	 	...
	 	if ( $res=$DbExt->rst($stmt)){		
		    ...
		    //     if ($search_mode!="postcode"){		        
			   ...
			   //          }
				  ...
				  //   	$distance=AddonMobileApp::t("")."".$distance ." $distance_type";
	     ...
	     //                  $distance_type_raw);		
		    ...
		    //     } /*end search type*/
	         ...
	         //     	);
		        ...
		        //dump($delivery_fee);        
	 			...
	 			$data[]=array(
	 			  ...
	 			  //'delivery_fee'=>!empty($delivery_fee)?$delivery_fee:AddonMobileApp::t("Free"), 
	 			  ...
	 			  'merchant_photo_bg'=>getOption($val['merchant_id'],'merchant_photo_bg'),
	 			  'offers'=>AddonMobileApp::getMerchantOffers($val['merchant_id']),
	 			  'service'=>$val['service'],
	 			  //'services'=>AddonMobileApp::displayServicesList($val['service']),
	 			  'services'=>$services,
	 			  //'distance'=>$distance,
	 			  //'delivery_estimation'=>$this->t("")."".getOption($mtid,'merchant_delivery_estimation'),
	 		...
	 		$data_sort = FunctionsV3::sortBy('tag_raw', $data, 'desc');
	...
	{	
		...
		if ( $data = AddonMobileApp::merchantInformation($this->data['merchant_id'])){				
			...
			$data['two_flavor_option'] = getOption($mtid,'merchant_two_flavor_option');
	...
	/* V 1.10 */
	public function actionLoadDeliveryOpt(){
		...
		$mtid=$this->data['merchant_id'];
		/*SET TIME*/
	    AddonMobileApp::setMerchantTimeZone($mtid);
		...
		if(!empty($cart)){
			...
			if(is_array($cart) && count($cart)>=1){
			   foreach ($cart as $keycart=>$cart_val) {
			   	    ...
			   	    if (is_array($new_cart) && count($new_cart)>=1){
			   	    	...
			   	    	$s_id = false; $s_price=false; $s_cookingref=false;
			   	    	$s_ingredients = false; $s_addon=false;
			   	    	...
			   	    	foreach ($new_cart as $key_cart=>$val_cart) {		
			   	    		...
			   	    		if (is_array($cart_val['ingredients']) && count($cart_val['ingredients'])>=1){
			   	    			if (is_array($val_cart['ingredients']) && count($val_cart['ingredients'])>=1){
			   	    				$i_1 = count($cart_val['ingredients']); $i_2 = count($val_cart['ingredients']);
			   	    				//dump($i_1); dump($i_2);
			   	    				if ( $i_1 == $i_2){			   	    					
			   	    					$flag_ingredients = 0;
			   	    		...
			   	    		if (is_array($cart_val['sub_item']) && count($cart_val['sub_item'])>=1){
			   	    			if (is_array($val_cart['sub_item']) && count($val_cart['sub_item'])>=1){
			   	    				$sub_1 = count($cart_val['sub_item']); $sub_2 = count($val_cart['sub_item']);
			   	    				if ($sub_1 == $sub_2){
			   	    					$flag_addon = 0;			   	    					
			   	    		...
			   	    		if($s_id==true && $s_price==true && $s_cookingref==true 
			   	    		&& $s_ingredients == true && $s_addon == true){
			   	    			$found_key=$key_cart;
			   	    			break;			   	    			
			   	    		}   	    	
			   	    	} /*END FOREACH*/
			   	    ...
			   	    } else $new_cart[$keycart]=$cart_val;
			   	}	
			}		
			...
			foreach ($cart as $val) {
		    	...
		    	$discounted_price=0; $subtotal_temp = 0;
		    	if ($val['discount']>0){
			...
			if (is_array($cart) && count($cart)>=1){
				...
				switch ( $this->data['transaction_type']) {
					case "delivery":						
					    ...
					    if ($search_mode!="postcode"){
					    	if(is_array($distance_new) && count($distance_new)>=1){					    	   
					    		$min_fees=FunctionsV3::getMinOrderByTableRates($mtid,
								   isset($distance_new['distance'])?$distance_new['distance']:'',
								   isset($distance_new['distance_type_raw'])?$distance_new['distance_type_raw']:'',
								   $minimum_order
								);	
								//dump($min_fees);
					    ...
					    $delivery_options['label']['date']=t("Delivery Date");
					    $delivery_options['label']['time']=t("Delivery Time");
					...
					case "pickup":	
					    ...
					    $delivery_options['label']['date']=t("Pickup Date");
					    $delivery_options['label']['time']=t("Pickup Time");
					...
					case "dinein":   
					    ...
					    if(is_numeric($maximum_order_dinein)){				    	
					    	if ($subtotal>$maximum_order_dinein){
					    		$validation_msg=$this->t("sorry but the maximum dinein order is")." ".
					    		AddonMobileApp::prettyPrice($maximum_order_dinein);
					    	}				    
					    }
					    
					    $delivery_options['label']['date']=t("Dinein Date");
					    $delivery_options['label']['time']=t("Dinein Time");
					    break;
			    ...
			    $this->details['estimation_delivery_time']=date('h:i A');
			    			    		   
			    $default_value_date = date("Y-m-d");
			    if($res_date = FunctionsV3::getDateList($mtid)){
			    	if(is_array($res_date) && count($res_date)>=1){
			    	   	foreach ($res_date as $res_date_key=>$res_date_val) {
			    	   		$default_value_date = $res_date_key;			    	   		
			    	   		break;
			    	   	}
			    	}			    
			    }			    
			    $delivery_options['default_value_date']=array(
			      'date'=>$default_value_date,
			      'date_pretty'=>FunctionsV3::prettyDate($default_value_date),
			    );
			    $delivery_options['default_value_time']=array(
			      'time'=>date("h:i A"),
			      'time_pretty'=>FunctionsV3::prettyTime(date("c")),
			    );			    
			    $this->details['delivery_options']=$delivery_options;
	...
	{				
			...
			if (is_array($cart) && count($cart)>=1){
		        ...
		        if ($search_mode!="postcode"){	
				    ...
				    if ( $this->data['transaction_type']=="delivery" ){		
				    	...
				    	/* PRUEBA CART */
				    	if( $this->data['google_lat'] != "" && $this->data['google_lng'] != "" ){
				    	    $cli_lat = $this->data['google_lat'];
        			        $cli_long = $this->data['google_lng'];
				    	}else{
				    	    if($this->data['formatted_address']!=''){
            			        $cli_geo=Yii::app()->functions->geodecodeAddress($this->data['formatted_address']);    
            			    }else{
            			        $formated_adr = $this->data['street'].'%2C+'.$this->data['city'].'%2C+'.$this->data['zipcode'].'%2C+'.$this->data['state'];
            			        $cli_geo=Yii::app()->functions->geodecodeAddress($formated_adr);
            			    }
            			    
            			    $cli_lat = $cli_geo['lat'];
        			        $cli_long = $cli_geo['long'];
				    	}
        			    ...
        			    //$distance_exp=3959;
        			    $distance_exp=6371;
        				// if ($distance_type_raw=="km"){
        				// 	$distance_exp=6371;
        				// }
        				
        				$stmt="
        					SELECT ( $distance_exp * acos( cos( radians($cli_lat) ) * cos( radians( latitude ) ) 
        					* cos( radians( lontitude ) - radians($cli_long) ) 
        					+ sin( radians($cli_lat) ) * sin( radians( latitude ) ) ) ) 
        					AS distance
        					FROM {{view_merchant}} a 
        					WHERE a.merchant_id = $mtid
        					";
        					
        				$distance=0;
        				$delivery_charges = 0;
        				$DbExt=new DbExt;	
        				if ($result=$DbExt->rst($stmt)){
        				    if(is_numeric($result[0]['distance'])){
        				        $distance=array(
                                    'unit'=>$distance_type_raw,
                                    'distance'=>$result[0]['distance'],
                                );
                                
                                $delivery_charges = FunctionsV3::getDeliveryChargesByPackage($distance['distance']);
        				    }else{
        				        $ok_distance=1;
        				    }
        				}else{
        				...
        				if($this->data['coverage_sector'] == "other"){
        				    $ok_distance = 1;
        				}
               ...
               /*get merchant offers*/
		        ...
		        if ($merchant_packaging_charge>0){
		        	...
		        	if($food['packaging_fee']!=0 || $food['packaging_fee']!=''){
		        		$merchant_packaging_charge = $food['packaging_fee'];

		        		if($food['packaging_incremental'] == 1) {
		        			$merchant_packaging_charge=$merchant_packaging_charge*$item_total;		
		        		}
	           ...
	           /*get the tax*/
		        ...
		        if ( $merchant_tax>0){
		        	...
		        	if ( $merchant_tax_charges==2){
		        	    // TAX - PACKAGE CHARGE
		        		// $tax=($taxable_total+$merchant_packaging_charge+($taxable_total*0.05))*($merchant_tax/100);
		        		$tax_calc=($taxable_total+$delivery_charges+$merchant_packaging_charge+($taxable_total*0.05))*($merchant_tax/100);
		        	} else {
		        		// TAX - PACKAGE CHARGE
		        		// $tax=($taxable_total+$delivery_charges+$merchant_packaging_charge+($taxable_total*0.05))*($merchant_tax/100);
		        		$tax_calc=($taxable_total+($taxable_total*0.05))*($merchant_tax/100);
		        	}
		        }		
				...
				if ($cargos){ //mi funcion bella
					$cart_final_content['cargos']=array(
					  'amount'=>$cargos,
					  'amount_pretty'=>AddonMobileApp::prettyPrice($cargos)
					);					
				}
				// if ($tax>0){
				...
				$grand_total=$subtotal+$delivery_charges+$merchant_packaging_charge+$tax+$tips_amount+$cargos; //momento eureka
				$cart_final_content['grand_total']=array(
				  'amount'=>$grand_total,
				  'amount_pretty'=>AddonMobileApp::prettyPrice($grand_total),
				  'amount_usd'=>FunctionsV3::getTotalAmountUSDByPackage($grand_total)
				);
				...
				if ( $this->data['transaction_type']=="delivery"){					
					if ($ok_distance==1){
						...
						$validation_msg=AddonMobileApp::t("Sorry, but you are not in the coverage area");
					}
				}
				...
				if ($has_pts==2){
					...
					} else $sub_total_based_earning = unPrettyPrice($cart_final_content['sub_total']['amount']);
		...
		if($client){
			if(isset($this->data['identification_card'])){
				if((strlen($this->data['identification_card']) > 6) && (strlen($this->data['identification_card']) < 10)){
					$functionk=new FunctionsK();
					$this->details['existe_ced_otros']=$functionk->CheckIdentificationCard($this->data['identification_card'],$client['client_id']);
					if(!$functionk->CheckIdentificationCard($this->data['identification_card'],$client['client_id'])){
					    $this->details['existe_ced_cliente']=$functionk->CheckIdentificationCardLoaded($this->data['identification_card'],$client['client_id']);
						if(!$functionk->CheckIdentificationCardLoaded($this->data['identification_card'],$client['client_id']))
					    $this->details['var_b']=$functionk->CheckIdentificationCardLoaded($this->data['identification_card'],$client['client_id']);
					    $this->details['var_f']='LOADED';
					    $DbExt=new DbExt;
						$DbExt->updateData("{{client}}",array(
						  'id_card'=>$this->data['identification_card'],
		...
		if ( $this->data['transaction_type']=="delivery"){
		    ...
		    if ($this->data['save_address']==2){		    	
		    	if($client){
		    		...
		    		if($search_mode=="postcode" && $app_version>=2.6){
		    			$params_address_location = array(
		    			  'client_id'=>$client['client_id'],
		    			...
		    			if($res_country = FunctionsV3::getStateByID($this->data['state_id'])){
				           $params_address_location['country_id']=$res_country['country_id'];
			            }
		    			$DbExt->qry("UPDATE 
			    		...
			    		");
		    			$DbExt->insertData("{{address_book_location}}",$params_address_location);
		    		} else {		    				    		
			    		$DbExt->qry("UPDATE 
			    		    {{address_book}}
			    		    SET as_default='1'
	...
	public function actionCheckOut()
	{
       ...
       if ( isset($this->data['delivery_time'])){
       	  if(!empty($this->data['delivery_time'])){
       	  	 ...
       	  	 if($delivery_asap==1){
       	  	 	...
       	  	 	$time_2=$this->data['delivery_date']." ".$timenow;       	  	 	       	  	 	
       	  	 } else $time_2=$this->data['delivery_date']." ".$this->data['delivery_time'];       
	   ...
	   if ( $resp=AddonMobileApp::getClientTokenInfo($this->data['client_token'])) {	   	  	   	  
	   	  ...
	   	  if($search_mode=="postcode"){
	   	  	 $address_book=array();
	   	  } else $address_book=AddonMobileApp::getDefaultAddressBook($resp['client_id']);	   	  
	   }	
	...
	{	
		...
		if ($Validator->validate()){
	        ...
	        /*NUEVO*/
	        /*check if id card already exist*/
	        $functionk=new FunctionsK();
	        if ( $functionk->CheckIdentificationCard($this->data['identification_card'])){
	        	$this->msg=$this->t("Sorry but your identification card is already exist in our records");
	        	$this->output();
	        }
	        if ( !$res=Yii::app()->functions->isClientExist($this->data['email_address']) ){
	    		...
	    		if ( $DbExt->insertData("{{client}}",$params)){
	    			...
	    			//update device client id
		   	   	   if (isset($this->data['device_id'])){
		   	   	   	   $app_version=AddonMobileApp::currentVersion();
		   	   	   	   if(isset($this->data['app_version'])){
		   	   	   	   	  if(!empty($this->data['app_version'])){
		   	   	   	   	  	$app_version = trim($this->data['app_version']);
		   	   	   	   	  }
		   	   	   	   }		
		   	   	       AddonMobileApp::registeredDevice($client_id,$this->data['device_id'],$this->data['device_platform'],$app_version);
		   	   	   }
	...
	{		
			...
			if ( $this->data['transaction_type']=="delivery" ){
				...
				if($distance_new=AddonMobileApp::getDistanceNew($merchant_info,$client_address)){
		    	   ...
		    	   if(is_array($distance_new) && count($distance_new)>=1){
					    ...
					    $distance_new['distance_type_raw'],
					    $minimum_order
					  );
					  					  
					  $cart_subtotal = isset($this->data['cart_subtotal'])?$this->data['cart_subtotal']:0;
					  					  
					  if($cart_subtotal>0 && $min_fees>0){
					  	 if($min_fees>$cart_subtotal){
					  	 	 $this->msg = $this->t("Sorry but Minimum order is")." ".Yii::app()->functions->normalPrettyPrice($min_fees);
					  	     $this->output();	
					  	 }
					  }					  
		    	   }	
		    	   /*END CHECK MINIMUM ORDER TABLE*/			
		...
		'wing','ipay_africa','strip_ideal','mol','dixipay','hubtel','trz','usc','mpt');		
		...
		if (AddonMobileApp::isArray($payment_list)){			
			foreach ($mobile_payment as $val) {				
				...
				if(array_key_exists($val,(array)$payment_list)){
					switch ($val) {						
						case "cod":			
						    $_label = $this->t("Transferencia bancaria");
						    if ($this->data['transaction_type']=="pickup"){
						    	$_label=$this->t("Transferencia bancaria");
						...
						case "pyp":		
						   ...
						   if($paypal_mobile_enabled=="yes" && !empty($paypal_client_id)){						   	   
						   	   ...
						   	   } else $paypal_lable = $this->t("Paypal");
						   	   $merchant_payment_list[]=array(
							      'icon'=>'fa-paypal',
							      'value'=>$val,
							      'label'=>$paypal_lable
						...
						case "pyr":	
						   ...
						   $_label=$this->t("Mis Pagos Mercantil");
						   if ($this->data['transaction_type']=="pickup"){
						...
						case "atz":							
							$merchant_payment_list[]=array(
									   'icon'=>'ion-card',
									   'value'=>$val,
									   'label'=>$this->t("MisPagos Mercantil")
									);
							break;
					   ...
					   case "stp":					   	
					    ...
					    case "rzr":		
					   	...
					   	case "strip_ideal":   
						    ...
						    $_label=$this->t("Transferencia Zelle");
			...
			if (AddonMobileApp::isArray($merchant_payment_list)){			
				...
				$this->details=array(
				  'transaction_type'=>$this->data['transaction_type'],
				  'show_mobile_number'=>$show_mobile_number,
				  'client_email_address'=>$client_email,
				  'client_contact_phone'=>$client_phone,
...
// 		die();
			...
			if ( $res=AddonMobileApp::computeCart($this->data)){				
				...
				if (empty($res['validation_msg'])){
				   ...
				   if (AddonMobileApp::isArray($json_data)) {
					   $cart=$res['cart'];
					   //dump($cart);
					   // defino el estado del pedido que sea PENDIENTE si está por transferncia
					   if ($this->data['payment_list']=="cod" || 
					      $this->data['payment_list']=="trz"  || 
					      $this->data['payment_list']=="usc"  || 
					      $this->data['payment_list']=="mpt"  || 
	    					...
	    					case "ip8":
	    						...
	    						case "mcd":	    							
							        ...
							        try {
							        	...
							        	$mp = new MP($this->details['mercapago']['mercado_id'] , 
							        	$this->details['mercapago']['mercado_key'] );	
	...
	{		
		...
		if ( $data = AddonMobileApp::merchantInformation($this->data['merchant_id'])){							
			...
			if ($payment_method=AddonMobileApp::getPaymentMethodNew($mtid, $data['service'])){								
		...
		if (isset($this->data['merchant_id'])){
			if ( $res=Yii::app()->functions->getReviewsList($this->data['merchant_id'])){
				...
				foreach ($res as $val) {
					...
					$replies = array(); $replies_data = array();
	...
	{		
		...
		$review_type = getOptionA('website_review_type');
		/*dump($review_type);
		dump($this->data);*/
		...
		if(isset($this->data['order_id'])){
		   if($this->data['order_id']>0){
		   	  if($order_info = Yii::app()->functions->getOrderInfo($order_id)){		   	  	 
		   	  	 $mtid=$order_info['merchant_id'];
		   	  }		   
		   }		
		}	
		
		if($mtid<=0){
			$Validator->msg[]=$this->t("Merchant id is missing");
		}	
		...
		if ( $Validator->validate()){
	    	...
	    	if($review_type==2) {	    		
	    	  ...
	    	  if($client_id>0 && $order_id>0){	    	  	  
		    	  if ($res = FunctionsV3::getReviewByOrder($client_id,$order_id)){			    	  	  
		    	  	  $this->msg = $this->t("This order has already rated");
		    	  	  $this->output();	
		    	  }	    	
	    	  }
	    	} else {
	...
	public function actionBrowseRestaurant()
	{
		...
		if (isset($this->data['restaurant_name'])){
			//$and=" AND restaurant_name LIKE '%". addslashes($this->data['restaurant_name']) ."%'";
			$and=" AND restaurant_name LIKE ".FunctionsV3::q("%".$this->data['restaurant_name']."%")." ";
		}	
		...
		if ($res=$DbExt->rst($stmt)){
			...
			foreach ($res as $val) {
				...
				$data[]=array(
	 			  ...
	 			  'services'=>AddonMobileApp::displayServicesList($val['service']),
	 			  'distance'=>'',
	 			  'delivery_estimation'=>AddonMobileApp::t("")."".getOption($mtid,'merchant_delivery_estimation'),
	 			  'delivery_distance'=>$delivery_distance,
	 			  'payment_available'=>$payment_available,
	 			  'is_sponsored'=>$val['is_sponsored']
	 			);
	...
	{		
		if ( $res=AddonMobileApp::getClientTokenInfo($this->data['client_token'])){
			...
			if(isset($this->data['identification_card'])){
			    $params['id_card']=$p->purify($this->data['identification_card']);
			}
			if(isset($this->data['birthday'])){
			    $params['birthdate']=$p->purify($this->data['birthday']);
			}
		...
		if ( $Validator->validate()){
		   ...
		   if ($res=$DbExt->rst($stmt)){
		   	   ...
		   	   if ($DbExt->updateData("{{client}}",$params,'client_id',$client_id)){
	   	             ...
	   	             'show_mobile_number'=>$show_mobile_number,
	   	             'social_strategy'=>'mobile',
	   	             'id_card'=>isset($res['id_card'])?$res['id_card']:'',
	   	             'birthdate'=>isset($res['birthdate'])?$res['birthdate']:'',
		   	   	   );
		   	   	   /*NUEVO*/		   	   	   
		   	   	   //update device client id
		   	   	   if (isset($this->data['device_id'])){		   	   	       
	...
	{		
		if ( $res=AddonMobileApp::getClientTokenInfo($this->data['client_token'])) {
			...
			$order = AddonMobileApp::clientHistyOrder($res['client_id']);
			...
			if($order){
				...
				foreach ($order as $val) {
					...
					$show_review = 0;
					if(FunctionsV3::canReviewOrder($val['status'])){
						if($val['review_count']<=0){
						   $show_review=1;
						}
					}				
										
					$show_cancel_order = 2; $cancel_status='';
					$cancel_class = 'tag_green';
					if(FunctionsV3::canCancelOrder($val['request_cancel'],$val['date_created'],$val['status'],$val['order_locked'],$val['request_cancel_status'])){
					  if($val['request_cancel']==1){
					  	 $cancel_status = t("Pending for review");					  	 
					  }	else $show_cancel_order=1;									  					  					  
					} else {
						if ($val['request_cancel_status']!='pending'){
				  	 	   $cancel_status=t("Request Cancel").":".t($val['request_cancel_status']);
				  	 	    if ($val['request_cancel_status']!="approved"){
				  	 	    	$cancel_class = 'tag_red';
				  	 	    }						
				  	    }					  
					}				
					...
					$data[]=array(
					  ...
					  'title_new'=>Yii::t("mobile","[transaction_type] #[order_id]",array(
					    '[transaction_type]'=>t($val['trans_type']),
					    '[order_id]'=>$val['order_id']
					  )),
					  'place_on'=>Yii::t("mobile","Place on [date]",array('[date]'=> Yii::app()->functions->translateDate(prettyDate($val['date_created'])))),
					  'payment_type'=>t(FunctionsV3::prettyPaymentTypeTrans($val['trans_type'],$val['payment_type'])),
					  'total_words'=>$this->t("Total"),
					  'total'=>FunctionsV3::prettyPrice($val['total_w_tax']),
					  ...
					  'rating'=>$val['rating']>0?$val['rating']:0
					);
				}
				$this->details=$data;
			 ...
			 if ( $res=AddonMobileApp::getOrderDetails($this->data['order_id'])){			 	  
			 	  ...
			 	  if ($history=FunctionsK::orderHistory($this->data['order_id'])){
			 	  	 foreach ($history as $val) {			 	  	 	
			 	  	 	...
			 	  	 	} else $new_remarks = !empty($val['remarks'])?$val['remarks']:'';
	...
	{		
		if ( $res=AddonMobileApp::getClientTokenInfo($this->data['client_token'])) {			 
			...
			if($search_mode=="postcode" && $app_version>=2.6){				
				if ($resp = AddonMobileApp::getAddressBookList($res['client_id'])){
					$this->code=1;
					$this->msg="OK";
					$this->details=$resp;
			 ...
			 if ( $resp=Yii::app()->functions->getOrderInfo($this->data['order_id'])){			 	  
			 	  ...
			 	  if(is_array($cart) && count($cart)>=1){
			 	  	 foreach ($cart as $cart_key=>$cart_val) {
			 	  	 	if ($item_details = Yii::app()->functions->getFoodItem($cart_val['item_id'])){	    			   			
			 	  ...
			 	  if ( $cart!=false){
			 	  	...
			 	  	if($found_not_available==TRUE){
			 	  		$this->msg= $this->t("The item you are about to re-order is not available");
			 	  	} else $this->msg=$this->t("something went wrong during processing your request");	 	  	
			 	  }			 
	...
	{		
		if (isset($this->data['lat']) && !empty($this->data['lng'])){			
			...
			if ($res=FunctionsV3::latToAdress($this->data['lat'],$this->data['lng'])){
				$this->code=1; $this->msg="OK";		
	...
	public function actionSaveSettings()
	{
		...
		if ( $client=AddonMobileApp::getClientTokenInfo($this->data['client_token'])) {			
			 ...
			 if ( $DbExt->updateData("{{mobile_registered}}",$params,'client_id',$client_id)){
			     ...
			     $this->details = array(
			       'enabled_push'=>empty($params['enabled_push'])?2:1
			     );
	...
	//		    'location_accuracy'=>!empty(getOptionA('mobileapp_location_accuracy'))?getOptionA('mobileapp_location_accuracy'):'false',
			    ...
			    'mobile_enabled_fblogin'=>getOptionA('mobile_enabled_fblogin'),
			    'mobile_auto_location'=>getOptionA('mobile_auto_location')
		    ),
		     'translation'=>$lang
		    );
	...
	public function actionPayAtz()
	{
		...
		if ( $resp=AddonMobileApp::getClientTokenInfo($this->data['client_token'])) {
            ...
            AuthorizePay::$mode = $mode_autho;     
            AuthorizePay::$api = $autho_api_id;
            AuthorizePay::$key = $autho_key; 
            
            $amount_to_pay  = number_format($this->data['total_w_tax'],2,'.','');
            
            if($resp = AuthorizePay::Paynow($this->data, $client_id)){
			    ...
			    $db_ext->insertData("{{payment_order}}",$resp);
...
public function actionCancelOrderCart()
{
	...
	{		
		...
		if ($Validator->validate()){
			...
			$params = array(
			 ...
			 } else $this->msg=HubtelPayments::$err;
	...
	{		
		...
		if ($state_id>=1){
			$DbExt=new DbExt;
			$stmt="
			SELECT city_id as id , name ,state_id
			FROM
			{{location_cities}}
			WHERE
			...
			if ( $res=$DbExt->rst($stmt)){
				...
				$this->code = 1; 
				$this->msg ="OK";
			} else $this->msg = AddonMobileApp::t("no city available");
		} else {			
	...
	public function actiongetLocationArea()
	{
		...
		if(isset($this->data['s'])){
		   if(!empty($this->data['s'])){
		   	  ...
		   	  $and.=" AND name LIKE ".FunctionsV3::q("%".$this->data['s']."%")." ";
		   }		
		}	
    	...
    	if ($res=$DbExt->rst($stmt)){
    		...
    		$this->msg="ok";
    		$this->details = $res;
    	} else $this->msg = AddonMobileApp::t("no location found");
		$this->output();
	}
	...
	public function actionlocationState()
	{
    	...
    	if ( $res=FunctionsV3::locationStateList($country_id)){
    		...
    		$this->msg="OK";
    		$this->details = $data;
    	} else $this->msg = $this->t("State not available");
		$this->output();
	}
	...
	public function actionGoogleLogin()
	{
	    ...
	    if ($Validator->validate()){
			...
			if ( $res=AddonMobileApp::checkifEmailExists($this->data['email'])){
				...
				if (empty($res['avatar'])){
					if($this->data['imageurl']!="undefined" && !empty($this->data['imageurl'])){
	...
	{	    
	    ...
	    if ( $res=AddonMobileApp::getCategoryCount($this->data['mtid'],$todays_day)){
	    	$this->code = 1;
	    	$this->msg = "OK";	    	
	    	...
	    	$this->code = 1;
	    	$this->msg = AddonMobileApp::t("Este establecimiento no tiene productos publicados.");
	    	$this->details = array(
	    	  'total'=>0,
	    	  'merchant_photo_bg'=>$merchant_photo_bg,
	    	  'restaurant_name'=>isset($merchant_info['restaurant_name'])?stripslashes($merchant_info['restaurant_name']):'',
	...
	public function actiongetCategory()
	{
		...
		/*SET TIME*/
		AddonMobileApp::setMerchantTimeZone($merchant_id);
        ...
        $and='';  $todays_day = date("l");                 
        $days=strtolower($todays_day); 
    	...
    	/*GET CLIENT INFO*/		
		$client=AddonMobileApp::getClientTokenInfo( isset($this->data['client_token'])?$this->data['client_token']:'' );
        $age = AddonMobileApp::getClientAge($client['client_id']);
		if($age >= 18){
	...
	{		
		...
		if ($res=$DbExt->rst($stmt)){
			...
			} else $this->msg = $this->t("No item found on this category");
		} else $this->msg = $this->t("No item found on this category");
	...
	public function actiongetItem()
	{
		...
		$DbExt=new DbExt; $p = new CHtmlPurifier();
		$data='';
		...
		} else $this->msg = $this->t("No item found in this category");
	...
	{		
		if (isset($this->data['page_id'])){
			...
			if ( $res = AddonMobileApp::getPagesByID($this->data['page_id'],$fields_required)){				
				...
				} else $data['content']=trim($res['content']);
				...
				$this->details=$data;
			} else $this->msg = $this->t("Page not available");
		} else $this->msg = $this->t("Page id is missing");
		$this->output();
	}
    ...
    {    	
    	...
    	if(is_numeric($id)){
    		...
    		if($res = $db_ext->rst($stmt)){
		    	...
		    	} else $this->msg = $this->t("Failed cannot delete review please try again later");
    		} else $this->msg = $this->t("Review id not found");
    	} else $this->msg = $this->t("Invalid review id");
    	$this->output();
    }
		...
		if(is_numeric($id)){
    		...
    		if($res = $db_ext->rst($stmt)){
    			...
    			if ($db_ext->updateData("{{review}}",$params,'id',$id)){
    				...
    				$this->msg = "ok";
    				$this->details='';
    			} else $this->msg = $this->t("update failed");
    		} else $this->msg = $this->t("Review id not found");
		} else $this->msg = $this->t("Invalid review id");    	
    	$this->output();
    }
    ...
    public function actionloadPhotos()
    {
    	...
    	if(is_numeric($merchant_id)){
            ...
            } else $this->msg = $this->t("gallery not available");
    	} else $this->msg = $this->t("Invalid merchant id");
    	$this->output();
    }
    ...
    public function actionuseThisLocation()
	{
		$this->actioncoordinatesToAddress();
	}
			...
			if ( $data=Yii::app()->functions->getOrder2($order_id)){
				 ...
				 if ( $json_details !=false){
				 	Yii::app()->functions->displayOrderHTML(array(
					  ...
					  $details[]=array('label'=>t("Name"),'value'=>$data['full_name']);
					  $details[]=array('label'=>t("Merchant Name"),'value'=>$data['merchant_name']);
					  $details[]=array('label'=>t("Telephone"),'value'=>$data['merchant_contact_phone']);
					  $details[]=array('label'=>t("Address"),'value'=>$data['merchant_address']);
					  
					  $details[]=array('label'=>t("TRN Type"),'value'=> t($data['trans_type']) );
					  $details[]=array('label'=>t("Payment Type"),'value'=> t($data['payment_type']) );
					  $details[]=array('label'=>t("Reference #"),'value'=> $data['order_id'] );
					  
					  if(isset($data['payment_reference'])){
					  	if(!empty($data['payment_reference'])){
					  		$details[]=array('label'=>t("Payment Ref"),'value'=> $data['payment_reference'] );
					  	}					  
					  }				 
					  
					  $trn_date = date('M d,Y G:i:s',strtotime($data['date_created']));
	                  $trn_date = Yii::app()->functions->translateDate($trn_date);
					  $details[]=array('label'=>t("TRN Date"),'value'=> $trn_date );
					  ...
					  switch ($data['trans_type']) {
					  	case "delivery":
					  		$details[]=array('label'=>t("Delivery Date"),'value'=> Yii::app()->functions->translateDate(prettyDate($data['delivery_date'])) );
					  		$details[]=array('label'=>t("Delivery Time"),'value'=> Yii::app()->functions->timeFormat($data['delivery_time']) );
					  		$details[]=array('label'=>t("Deliver to"),'value'=> !empty($data['client_delivery_address'])?$data['client_delivery_address']:'' );
					  		$details[]=array('label'=>t("Delivery Instruction"),'value'=> !empty($data['delivery_instruction'])?$data['delivery_instruction']:'' );					  		
					  		$details[]=array('label'=>t("Location Name"),'value'=> !empty($data['location_name'])?$data['location_name']:'' );
					        $details[]=array('label'=>t("Contact Number"),'value'=> !empty($data['contact_phone'])?$data['contact_phone']:'' );
					  		break;
					  		
					    case "pickup":
					  		$details[]=array('label'=>t("Contact Number"),'value'=> $data['contact_phone'] );
					  		$details[]=array('label'=>t("Pickup Date"),'value'=> Yii::app()->functions->translateDate(prettyDate($data['delivery_date'])) );
					  		$details[]=array('label'=>t("Pickup Time"),'value'=> Yii::app()->functions->timeFormat($data['delivery_time']) );
					  		break;
					  		
					  	case "dinein":
					  		$details[]=array('label'=>t("Contact Number"),'value'=> $data['contact_phone'] );
					  		$details[]=array('label'=>t("Dine in Date"),'value'=> Yii::app()->functions->translateDate(prettyDate($data['delivery_date'])) );
					  		$details[]=array('label'=>t("Dine in Time"),'value'=> Yii::app()->functions->timeFormat($data['delivery_time']) );
					  		$details[]=array('label'=>t("Number of guest"),'value'=> !empty($data['dinein_number_of_guest'])?$data['dinein_number_of_guest']:'' );
					  		$details[]=array('label'=>t("Special instructions"),'value'=> !empty($data['dinein_special_instruction'])?$data['dinein_special_instruction']:'' );
					  		break;
					  ...
					  $html =  Yii::app()->functions->details['html'];					  
				 ...
				 } else $this->msg = $this->t("Failed loading order details");
	...
	public function actionrequestCancelOrder()
	{		
	...
	public function actionloadOptionList()
	{		
		...
		$option_type = isset($this->data['option_type'])?$this->data['option_type']:'';
		if(!empty($option_type)){
			switch ($option_type) {
				case "DateList":
					$res = FunctionsV3::getDateList($mtid);
					$this->code = 1;
					$this->msg = "OK";
					$this->details = array(
				...
				case "TimeList":	
				   ...
				   if(isset($this->data['delivery_date'])){
				   	  if(!empty($this->data['delivery_date'])){				   	  	
				   	  	  $now = $this->data['delivery_date'];
				   	  }
				   }				   
				   $res = FunctionsV3::getTimeList($mtid,$now);	   
				   $this->code = 1;
					$this->msg = "OK";
					$this->details = array(
					  'option_type'=>$option_type,
				...
				default:
					$this->msg = $this->t("Option type is not defined");
					break;
			}
		} else $this->msg = $this->t("Option type is not defined");
		$this->output();
	}
	
	public function actiongetPostCodeAddressBookDetails()
	{	
		$client=AddonMobileApp::getClientTokenInfo( isset($this->data['client_token'])?$this->data['client_token']:'' );
		if(!$client){
			$this->msg=$this->t("Token not valid");
		...
		$id = isset($this->data['id'])?$this->data['id']:'';
		if($id>0){
			if ( $res = FunctionsV3::getAddressByLocationFullDetails($id)){
				 $this->code = 1;
				 $this->msg = "OK";
				 $this->details = array(
				   'data'=>$res
				 );
			} else $this->msg = $this->t("Record not found");
		} else $this->msg =	$this->t("Invalid record id");
		$this->output();
	}
	...
	{		
		...
		$Validator=new Validator;
		$req=array(		  
		  'street'=>$this->t("Street is required"),
		  'state_id'=>$this->t("State is required"),
		  'city_id'=>$this->t("City is required"),
		  'area_id'=>$this->t("Area is required"),		  
		);
		...
		if ($Validator->validate()){
			$params = array(
			  ...
			  'location_name'=>trim($this->data['location_name']),			  
			  'state_id'=>$this->data['state_id'],
			  'city_id'=>$this->data['city_id'],
			  'area_id'=>$this->data['area_id'],
			...
			if($res = FunctionsV3::getStateByID($this->data['state_id'])){
				$params['country_id']=$res['country_id'];
			}
			...
			if($params['as_default']==1){
			   $db->qry("UPDATE {{address_book_location}}
			    SET as_default='0'
			    WHERE
			...
			if($id>0){
				...
				if ( $db->updateData("{{address_book_location}}",$params,'id',$id)){
					$this->code = 1;
					$this->msg = $this->t("successfully updated");
				} else $this->msg = $this->t("Cannot update records");
			} else {
				if ($db->insertData("{{address_book_location}}",$params)){
					$this->code = 1;
					$this->msg = $this->t("Successful");
				} else $this->msg=AddonMobileApp::t("ERROR: Cannot insert records.");
			}		
			unset($db);
		} else $this->msg =$this->msg=AddonMobileApp::parseValidatorError($Validator->getError());
				
		$this->output();
	}
	
	public function actiondeleteAddressBookLocation()
	{		
		$client=AddonMobileApp::getClientTokenInfo( isset($this->data['client_token'])?$this->data['client_token']:'' );
		if(!$client){
			$this->msg=$this->t("Token not valid");
	...
	public function actiongetAddressBookLocation()
	{		
		$client=AddonMobileApp::getClientTokenInfo( isset($this->data['client_token'])?$this->data['client_token']:'' );
		if(!$client){
			$this->msg=$this->t("Token not valid");
		...
		$client_id = $client['client_id'];
		if ($resp = AddonMobileApp::getAddressBookList($client_id)){
			$this->code=1;
			$this->msg="OK";
			$this->details=$resp;
		} else $this->msg = $this->t("no results");	
	...
	public function actionsetAddressLocation()
	{
		$this->actiongetPostCodeAddressBookDetails();
	}
	...
	public function actionreRegisterDevice()
	{
		...
		$app_version=AddonMobileApp::currentVersion();
	   	...
	   	$new_device_id = isset($this->data['new_device_id'])?trim($this->data['new_device_id']):'';
	   	if(!empty($new_device_id)){
		   	$resp = AddonMobileApp::registeredDevice($client_id,$new_device_id,$this->data['device_platform'],$app_version);
		   	if($resp){
		   		$this->code = 1;
		   		$this->msg = "OK";
		   		$this->details = $new_device_id;
		   	} else $this->msg = $this->t("Failed registering device");
	   	} else $this->msg = $this->t("New device id is empty");
	   	$this->output();
	}
	...
	public function actiongetNotificationList()
	{
		...
		if($client_id>0){			
			$data = AddonMobileApp::getPushLogsByClientID($client_id);
		} else {			
		   	$data = AddonMobileApp::getPushLogsByDeviceID($device_id);
		}
